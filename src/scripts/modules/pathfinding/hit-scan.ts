...
export const getVisible = (tiles: Tile[], source: Tile, obstacles: Tile[]): Tile[] => {
	...
	for (const o of obstacles) {
		...
		const obstVec = Vector2D.fromTiles(source, o);
		...
		for (const tile of visible) {
			const tileVec = Vector2D.fromTiles(source, tile);
			...
			const tileAngle = Math.atan2(tileVec.y, tileVec.x) - Math.atan2(obstVec.y, obstVec.x);
