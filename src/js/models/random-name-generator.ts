...
export class RandomNameGenerator {
	...
	public static get(count: number, maxlength: number): string[] {
		...
		while (count--) {
			names.push( RandomNameGenerator.getRandom(graph, maxlength) );
		}
		return names;
	}

	private static getRandom(graph: IGraph, maxlength: number): string {
		let name = RandomNameGenerator.getRandomStarter(graph);
		...
		while (maxlength--) {
			const letter = RandomNameGenerator.getNextLetter(graph, name.substring(name.length - order));
	...
	private static createGraph(dict: string[], order: number): IGraph {
		const paramEnd = RandomNameGenerator.paramEnd;
		...
		RandomNameGenerator.setWeights(graph);

		return graph;
	}

	private static getRandomStarter(graph: IGraph): string {
	...
	private static getNextLetter(graph: IGraph, groupName: string): string {
		const paramEnd = RandomNameGenerator.paramEnd;
	...
	private static setWeights(graph: IGraph) {
